...
Table~\ref{tab:data} summarises the corpus. All trajectories come from the
\emph{same} agent so the nominal-versus-attack comparison is not confounded by
model identity.
...
AUROC is \(0.77\). The median detection delay is one action: when SENTRY fires,
