...
LLM-free signals---a tool-transition model, an argument-novelty term, and an
...
detects \(81\%\) of prompt-injection \emph{attempts} at a \(5.2\%\) false-alarm
rate with median delay one action, and we argue that injection-attempt
detection, not hijack-success prediction, is the correct guardrail target. We
give an honest ablation---including three signals we implemented and
\emph{rejected}---and are explicit about the ceiling imposed by small,
free-tier data. We frame this as a preliminary empirical trace detector
with a proposed validity framework.}
...
\item \textbf{Three cheap, LLM-free signals and two principled combiners.}
...
\item \textbf{A preliminary empirical trace detector with an operational reframe.} On real AgentDojo and
\(\tau\)-bench trajectories, SENTRY detects \(81\%\) of injection
\emph{attempts} at a \(5.2\%\) false-alarm rate with median delay one action
(Section~\ref{sec:results}). We show that injection-\emph{attempt} detection,
not hijack-\emph{success} prediction, is a more attainable monitor objective, though
we acknowledge this is a post-hoc target motivated by our data constraints.
...
explicit about the ceiling imposed by small, free-tier data
...
\(\Ecal_\nu\) the law with change at \(\nu\). Equation~\eqref{eq:arl} bounds
the expected time to the \emph{first} false alarm, which is not generally 
equivalent to a per-step false-alarm rate for dependent streams or repeatedly 
restarted detectors; our reported empirical metric (total alarms divided by 
total actions) is therefore a proxy.
...
increment. We acknowledge that our empirical implementation estimates means 
and variances directly from calibration traces without a proven sub-Gaussian bound, 
and our null class is therefore implicitly restricted to processes satisfying 
this learned baseline-increment condition rather than all possible benign agents.
...
merged increment is the weighted arithmetic mean (an admissible 
merge of e-values~\cite{vovk2021evalues}),
\begin{equation}
...
scales to a common e-value scale, and exposes per-signal attribution. We note, 
however, that marginal conformal validity assumes exchangeability, which is 
insufficient for the conditional expectation required by a strict e-detector 
when allowing arbitrary history dependence.
...
shin2023edetectors}). We explicitly note that over an infinite deployment with 
repeated resets, even a valid detector will eventually false-alarm with probability 
one; repeated restart guarantees \emph{conditional} ARL between alarms, not global 
probability of no false alarm forever.
...
increment, the worst-case average detection delay obeys a bound inversely 
proportional to \(D(\Qcal\Vert\Pcal)\), the Kullback--Leibler separation 
between the post- and pre-change score laws achievable by the mixture.
\end{proposition}

While \(D(\Qcal\Vert\Pcal)\) is technically undefined for the broad, implicit 
distribution classes we employ, and this exponential-family delay result is not 
formally established for our conformal combiner, Proposition~\ref{prop:delay} 
nevertheless formalizes the intuition driving SENTRY's signal design: delay 
shrinks as separation grows. This is why the instruction-likeness term yields 
a one-action median delay empirically.
...
exact Ville threshold \(1/\alpha\) need not control the false-alarm rate. We adopt the PAC-thresholding correction of E-valuator~\cite{sadhuka2025evaluator},
adapted to the e-detector's running maximum on finite trajectories. On a held-out 
nominal split \(\Dcal_{\mathrm{thresh}}\) of \(m\) trajectories we run the detector to
completion, record each trajectory's running maximum \(M^{(i)}\), and aim to set the
threshold to an order statistic chosen by a Binomial-tail (Wilks) bound. However, we 
explicitly note that our protocol has only \(m=5\) threshold trajectories. For our 
targets \((\alpha=0.2, \delta=0.2)\), no order statistic satisfies the confidence 
requirement; the implementation falls back to the sample maximum. Thus, our reported 
PAC results are technically uncertified. Furthermore, PAC guarantees strictly apply 
only to the finite horizons observed during calibration; true infinite monitoring 
would require the Ville threshold without PAC corrections.
...
\textbf{Bigram+nov+inj (summed)}  & \textbf{0.87} & \textbf{0.81\,$\pm$\,0.00} & \textbf{0.73} & \textbf{0.62\,$\pm$\,0.00} & 0.052 \\
...
\(81\%\) of injection \emph{attempts} at a \(5.2\%\) per-step false-alarm rate,
...
Bigram+nov+inj (summed)          & \textbf{0.82\,$\pm$\,0.02} & \textbf{0.64\,$\pm$\,0.04} & 0.116 \\
...
Table~\ref{tab:main} (\(0.81\) vs \(0.62\) detection) is not cosmetic. On our
data, roughly \(11\) of the \(16\) AgentDojo-``successful'' attacks contain
...
slack     & 0 & 2 & 2 \\
\midrule
\textbf{total} & \textbf{12} & \textbf{16} & \textbf{16} \\
\botrule
\end{tabular}
\end{table}
...
contribution of each design choice. The original hash+Gaussian surprise is
near chance (AUROC \(0.40\) attempt, \(0.05\) detection): its tool term
conditioned on task identifiers that are always unseen at test time. Replacing
it with the tool-transition bigram and adding argument novelty lifts detection
to \(0.46\) (attempt); adding the observation instruction-likeness term lifts
it to \(0.81\)---by far the largest single jump. Data-flow taint and goal grounding
were implemented but rejected, as they did not separate on our free-tier data,
and are not shipped in the final model. The instruction weight is not
knife-edge tuned: Figure~\ref{fig:robust} shows AUROC is flat for any weight in
\([2,16]\), and likewise for the novelty weight, though we caution that our
headline signal was developed on this evaluation corpus without a preregistered
development set, presenting a risk of feature-selection leakage.
...
more \emph{powerful} here (\(0.81\) vs \(0.75\) attempt detection); the
...
operating point when the formal guarantee is valued over raw power. Which
combiner to deploy is a genuine engineering choice, and SENTRY ships both.

\subsection{Signals that did not work (honest negatives)}
...
\begin{itemize}
\item \textbf{Data-flow taint} (AUROC \(0.47\)). The strongest published
trace-only detector, AgentArmor~\cite{wang2025agentarmor}, and the
provenance-by-design of CaMeL~\cite{debenedetti2025camel} key on the fact that
an argument value entering via an untrusted observation but absent from the
user request is attacker payload. We scored exactly this. It failed on our
data because our weak free-tier ``successful'' attacks rarely execute a
payload-bearing tool call at all (only \(4/27\) copy a novel observation token
into an argument); the harm often surfaces as a text reply. This is a property
of the data, not the signal, and we expect it to recover on
capable-agent traces---a concrete prediction for future work.
...
cannot do, and because two of the three are predicted to behave differently on
stronger data---a falsifiable claim rather than a silent omission.
...
(green) and the two rejected ones (grey). The rejected signals sit at the
chance line on our data; the discussion explains why, and when they are
expected to recover.}\label{fig:signals}
\end{figure}
...
FPR~\cite{wang2025agentarmor}, SENTRY's \(81\%\) attempt detection is lower,
...
\paragraph{Limitations.} We are deliberate about these. \emph{(i)~The PAC
threshold is saturated} at our calibration size (\(m=5\)): with so few
threshold trajectories the Binomial-tail order statistic reduces to the sample
maximum, so the PAC guarantee~\eqref{eq:pac} is not yet certified at a useful
\((\alpha,\delta)\); more calibration data is the direct fix and the single
biggest gap between having the theory and demonstrating it. \emph{(ii)~Data
quality caps detection}: \(\sim\!11/16\) ``successful'' attacks have no
observable injection because the free-tier agent's successes are degenerate.
\emph{(iii)~Experimental confounding}: attacks were not resampled across splits,
leading to artificially zero cross-split variance. There is severe domain and length
confounding (attacks average 2.6 actions vs 5.3 for nominal), and our free-model router
confounds detector performance with varying model identities. \emph{(iv)~Lack of baselines}:
we do not include external raw baselines (e.g. LLM judge, CUSUM anomaly detector), and the
AgentArmor comparison uses a different denominator. \emph{(v)~Data-dependent tuning}:
e-value mixture weights were chosen post-hoc on the evaluation corpus, and our headline
signal was developed on the evaluation corpus without preregistration, presenting a risk of
feature-selection leakage. \emph{(vi)~API and Synthetic limits}: our public API requires
precomputed scalars and does not implement the instrumentation hook required for true ``zero-delay'',
while our synthetic experiment validates only a finite-horizon Gaussian simulation, not
arbitrary multi-task dependence.
re-test the taint and grounding signals under the conditions where the
literature reports they succeed.
...
\paragraph{Capable-agent data (highest priority).} Every ceiling we hit traces
to the free-tier agent: degenerate ``successful'' attacks with no observable
injection, and a calibration set too small to certify the PAC threshold.
Re-collecting AgentDojo and \(\tau\)-bench trajectories from a strong,
paid agent would raise the attainable detection ceiling and, critically,
create the conditions under which the two provenance signals we rejected are
predicted to succeed---a falsifiable test of Section~\ref{sec:ablation}'s
explanation.

\paragraph{A certified false-alarm guarantee.} With more threshold
trajectories the Binomial-tail order statistic becomes non-degenerate and the
PAC bound~\eqref{eq:pac} is certified at a useful \((\alpha,\delta)\). This is
the single largest gap between SENTRY's theory and its current demonstration.
...
real AgentDojo and \(\tau\)-bench trajectories it detects \(81\%\) of
prompt-injection attempts at a \(5.2\%\) false-alarm rate with a one-action
median delay, and we argued that injection-attempt detection is an attainable
target for a guardrail. Our ablation is honest about three signals that did
not help and about the data-driven ceiling on our numbers, framing this as a 
preliminary empirical trace detector. The clear next step
is a capable-agent data collection, which we expect to lift the ceiling and
to revive the provenance-based signals; longer term, replacing the lightweight
...
\item \textbf{Data governance and privacy.} The released logs contain account-like 
identifiers and user/task content from the underlying benchmark suites. We release 
them solely for reproducibility under the terms of the original datasets, but make no 
further privacy guarantees.
\end{itemize}
