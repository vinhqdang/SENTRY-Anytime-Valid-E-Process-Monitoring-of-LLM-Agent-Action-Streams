...
detects \(100\%\) of prompt-injection attempts \emph{whose payload is
observable in the black-box trace} (\(24/32\) attacks) at a \(5\%\) per-step
false-alarm rate with a one-action median delay, and \(81\%\) of all attempts
including \(8/32\) with no trace-observable payload; we argue that
injection-attempt detection, not hijack-success prediction, is the correct
guardrail target. We give a full ablation---including three signals we
implemented and \emph{rejected} because they did not separate on our
data---and are explicit about the limitations imposed by small, free-tier
data. To our knowledge this is the first anytime-valid change-detection study
...
\(\tau\)-bench trajectories, SENTRY detects \(100\%\) of injection attempts
whose payload is observable in the trace, and \(81\%\) of all attempts, at a
\(5\%\) false-alarm rate with a one-action median delay
...
black-box trace (24/32), all injection \emph{attempts} (32/32), and
\emph{hijack success} (the 16 attacks AgentDojo labels compromised). The
observable-injection target is the fair measure of a trace-only monitor;
detection there is perfect and has zero cross-split variance.}
\label{tab:main}
\begin{tabular}{@{}lcccc@{}}
\toprule
Configuration & Observable det. & Attempt det. & Hijack det. & FAR \\
\midrule
Hash + Gaussian (summed)          & 0.07\,$\pm$\,0.08 & 0.05\,$\pm$\,0.06 & 0.03\,$\pm$\,0.03 & 0.018 \\
\textbf{Bigram+nov+inj (summed)}  & \textbf{1.00\,$\pm$\,0.00} & \textbf{0.81\,$\pm$\,0.00} & \textbf{0.62\,$\pm$\,0.00} & 0.052 \\
Bigram+nov+inj (e-value mix)      & 0.95\,$\pm$\,0.08 & 0.75\,$\pm$\,0.08 & 0.53\,$\pm$\,0.11 & 0.062 \\
\botrule
\end{tabular}
\\[3pt]
{\footnotesize Threshold-free separability (AUROC, attempt vs benign): 0.40,
\textbf{0.87}, 0.79 for the three configurations respectively.}
\end{table}
...
surfaces in the observation stream (\(24/32\))---the three-signal summed model
detects \(100\%\) at a \(5.2\%\) per-step false-alarm rate, with \emph{zero}
variance across the ten splits. Counting \emph{all} \(32\) attempts, including
\(8\) whose payload never appears in the trace (Section~\ref{sec:reframe}),
detection is \(81\%\); on the harder hijack-success target it is \(62\%\), and
the threshold-free attempt-vs-benign AUROC is \(0.87\). The median detection
...
full delay distribution---concentrated at zero to one step for the
...
targets in Table~\ref{tab:main} (\(1.00\) observable vs \(0.81\) all-attempts
vs \(0.62\) hijack detection) is not cosmetic. Of the \(32\) attack
trajectories, \(8\) contain \emph{no} injected instruction anywhere in the
parsed observation stream: the weak free-tier agent's ``success'' was
degenerate, or the payload sat in a field it never surfaced. No trace-only
signal can (or arguably should) fire on those, which is exactly why the
all-attempts number (\(0.81\)) sits below the observable-injection number
(\(1.00\))---the difference is entirely those \(8\) undetectable-by-construction
cases, not a failure of the detector. Reporting both, rather than only the
more favourable observable-injection figure, is the honest presentation; the
observable-injection target is nonetheless the fair measure of what a
trace-only monitor can achieve, and there SENTRY is perfect at a \(5\%\)
false-alarm rate.
...
near chance (attempt-vs-benign AUROC \(0.40\), \(0.05\) detection): its tool
term conditioned on task identifiers that are always unseen at test time.
Replacing it with the tool-transition bigram and adding argument novelty lifts
attempt detection to \(0.46\); adding the observation instruction-likeness
term lifts it to \(0.81\) all-attempts (and \(1.00\) on the observable
subset), AUROC \(0.87\)---by far the largest single jump. The instruction
weight is not knife-edge tuned: Figure~\ref{fig:robust} shows AUROC is flat for
any weight in \([2,16]\), and likewise for the novelty weight.
...
from the user request is attacker payload. We scored exactly this. It failed on
our data because our weak free-tier ``successful'' attacks rarely execute a
...
of the data, not the signal, and we expect it to recover on capable-agent
traces---a concrete, falsifiable prediction for future work.
...
and the two rejected ones. The rejected signals sit at the chance line on our
data; Section~\ref{sec:negatives} explains why, and when they are expected to
recover.}\label{fig:signals}
\end{figure}
...
FPR~\cite{wang2025agentarmor}, SENTRY's \(100\%\) detection on the
observable-injection subset (\(81\%\) over all attempts) is comparable on the
fair measure, and is obtained from \(\sim\!8\) nominal trajectories of
black-box logs, carries an anytime-valid ARL guarantee AgentArmor does not,
and is model-agnostic. Against detectors that use model internals or LLM
judges~\cite{belenky2026cheap,liu2025datasentinel,jia2024taskshield}, SENTRY
trades some power for a formal guarantee and near-zero cost. We stress the
comparison is indicative, not head-to-head: our corpus is small and generated
by weak free-tier agents (Section~\ref{sec:limitations}), so these numbers
establish feasibility of anytime-valid trace-only monitoring, not a ranking
against systems evaluated on larger, stronger-agent data.

\paragraph{Limitations.}\label{sec:limitations} We are deliberate about these.
\emph{(i)~The PAC threshold is saturated} at our calibration size (\(m=5\)
threshold trajectories): the Binomial-tail order statistic reduces to the
sample maximum, so the PAC guarantee~\eqref{eq:pac} is not yet certified at a
useful \((\alpha,\delta)\); more calibration data is the direct fix and the
single biggest gap between having the theory and demonstrating it.
\emph{(ii)~Data quality caps the all-attempts number}: \(8/32\) attacks have no
trace-observable injection because the free-tier agent's ``successes'' are
degenerate, which is exactly the \(81\%\) vs \(100\%\) gap of
Section~\ref{sec:reframe}. \emph{(iii)~Small samples}: \(n=24\) nominal and
\(32\) attack trajectories suffice to separate \(0.87\) AUROC from chance on
identical splits but leave wide intervals. \emph{(iv)~One attack family}: we
evaluate AgentDojo's \texttt{important\_instructions} attack; generality across
attack templates is untested. None of these are limitations of the method's
design; all are addressable with a paid, capable agent and more data, which we
identify as the highest-value next step and which would also let us re-test the
taint and grounding signals under the conditions where the literature reports
they succeed.
...
Re-collecting AgentDojo and \(\tau\)-bench trajectories from a strong, paid
agent would raise the all-attempts detection ceiling toward the \(100\%\) we
already achieve on the observable subset, and---critically---create the
conditions under which the two provenance signals we rejected
(Section~\ref{sec:negatives}) are predicted to succeed, a falsifiable test.
...
real AgentDojo and \(\tau\)-bench trajectories it detects \(100\%\) of
prompt-injection attempts whose payload is observable in the black-box trace
(\(81\%\) of all attempts) at a \(5\%\) false-alarm rate with a one-action
median delay, and we argued that injection-attempt detection is the right
target for a guardrail. Our ablation is honest about three signals that did
not help and about the data-driven ceiling on our numbers. The clear next step
is a capable-agent data collection, which we expect to lift the all-attempts
ceiling and to revive the provenance-based signals; longer term, replacing the
lightweight reference with a causal world model, and formalising the
multi-signal e-value mixture's detection-delay bound, are natural extensions.
