...
\author*[1]{\fnm{Quang-Vinh} \sur{Dang}}\email{ai-data-ai-presales@smartosc.com}
...
detects \(81\%\) of prompt-injection \emph{attempts} at a \(5\%\) false-alarm
...
free-tier data. To our knowledge this is the first anytime-valid
...
\emph{attempts} at a \(5\%\) false-alarm rate with median delay one action
...
\(\Ecal_\nu\) the law with change at \(\nu\). Equation~\eqref{eq:arl} is
equivalent to a per-step false-alarm rate at most \(\alpha\); it must hold
nonasymptotically, without i.i.d.\ assumptions, and under continuous
monitoring.
...
magnitude, since a convex combination of valid increments is a valid
increment.
...
\label{eq:evalue-mix}
\end{equation}
which is itself a valid increment, requires no tail assumption on any signal,
maps signals on different scales to a common e-value scale, and exposes
per-signal attribution (the dominant \(e_k\) at the alarm names the signal
that fired).
...
shin2023edetectors})---this is what lets a single monitor run across an
unbounded deployment rather than certifying one bounded trajectory. We record
the delay behaviour, again inherited rather than newly proved.
...
Equation~\eqref{eq:delay-bound} makes precise the intuition that drives
SENTRY's signal design: delay shrinks as \(D(\Qcal\Vert\Pcal)\) grows, so the
better a signal separates injected from nominal behaviour, the faster the
alarm. It is the theoretical reason the observation instruction-likeness
term---which places almost all of its nominal mass at zero and its attack mass
far above it (Section~\ref{sec:results})---yields the one-action median delay
...
threshold \(c_\alpha\) and the exact-Ville threshold \(1/\alpha\), and are
explicit (Section~\ref{sec:discussion}) about when \(\Dcal_{\mathrm{thresh}}\)
is too small for the bound to be non-vacuous.
...
it to \(0.81\)---by far the largest single jump. The instruction weight is not
knife-edge tuned: Figure~\ref{fig:robust} shows AUROC is flat for any weight in
\([2,16]\), and likewise for the novelty weight.
...
\emph{(iii)~Small samples}: \(n=16\) attacks suffices to separate \(0.87\) from
chance on identical splits but leaves wide intervals. \emph{(iv)~One attack
family}: we evaluate AgentDojo's \texttt{important\_instructions}; generality
across attack templates is untested. None of these are limitations of the
method's design; all are addressable with a paid, capable agent and more data,
which we identify as the highest-value next step and which would also let us
...
prompt-injection attempts at a \(5\%\) false-alarm rate with a one-action
