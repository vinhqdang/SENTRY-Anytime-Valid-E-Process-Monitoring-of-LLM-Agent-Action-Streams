...
\title{SENTRY: Anytime-Valid E-Process Monitoring of LLM Agent Action Streams}

\author{Quang-Vinh~Dang%
\thanks{Q.-V. Dang is with British University Vietnam, Hung Yen, Vietnam.
E-mail: vinh.dq4@buv.edu.vn.}%
\thanks{Code, the collected trajectory corpus, and scripts that reproduce every
number and figure are available at
\protect\url{https://github.com/vinhqdang/SENTRY-Anytime-Valid-E-Process-Monitoring-of-LLM-Agent-Action-Streams}.}}

\markboth{IEEE Transactions on Dependable and Secure Computing}%
{Dang: SENTRY: Anytime-Valid E-Process Monitoring of LLM Agent Action Streams}

\IEEEtitleabstractindextext{%
\begin{abstract}
Large language model (LLM) agents that call external tools are
...
false-alarm guarantee holds with finite trusted data. We give a full
ablation---including three signals we implemented and \emph{rejected} because
they did not separate on our data. A generalisation study shows the method
transfers cleanly to a second, unrelated agent (GPT-4o-mini, AUROC \(0.82\)) but
that the shipped signal is attack-specific: it detects imperative-style
injections near-perfectly yet fails on InjecAgent's data-integration payloads
(AUROC \(0.47\)), a boundary we report and explain rather than hide.
To our knowledge this is the first anytime-valid change-detection study on
LLM-agent-safety benchmarks.
\end{abstract}
\begin{IEEEkeywords}
Large language model agents, prompt injection, e-values, sequential change detection, anytime-valid inference, runtime monitoring
\end{IEEEkeywords}}
...
sanity check (figure in the supplementary material): (a) the empirical per-step
false-alarm rate tracks at or below the nominal \(\alpha\) across
\(\alpha\in\{0.01,\dots,0.2\}\) on pure-nominal streams under continuous
monitoring, and (b) on a stream with an injected mean shift the statistic is
flat before the change and crosses threshold within a few steps after. This
confirms the SR recursion, restart logic and PAC
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/delay.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/instruction_sep.pdf}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/ablation_progression.pdf}
...
\emph{A different agent.} Re-collecting the AgentDojo
benign-plus-\texttt{important\_instructions} corpus with a second, unrelated
agent (GPT-4o-mini), and re-fitting the reference on \emph{its} trajectories,
leaves detection essentially unchanged---marginally stronger: \(100\%\)
observable detection, \(77\%\) over all attempts, attempt-AUROC \(0.82\) (versus
\(0.77\)), at a \(4.6\%\) false-alarm rate. Because the reference is learned
per-deployment from the agent's own nominal behaviour, it transfers.

\emph{A different attack.} Holding the agent fixed and swapping the attack for
InjecAgent~\cite{zhan2024injecagent}---a data-integration injection phrased to
read like ordinary retrieved content---SENTRY \emph{fails}: attempt-AUROC falls
to \(0.47\) (chance), and only \(10\%\) of its injections are observable to the
instruction-likeness signal versus \(99\%\) for \texttt{important\_instructions}
(median instruction-likeness \(0.0\) versus \(4.1\)). The anytime-valid
\emph{framework} is attack-agnostic, but the particular trace-only \emph{signal}
is not: catching InjecAgent-style attacks needs a provenance- or
transformation-aware signal (Section~\ref{sec:future}), now motivated by direct
...
shows SENTRY detects imperative-style injections near-perfectly but essentially
fails on InjecAgent's data-integration payloads (AUROC \(0.47\)), whose phrasing
carries almost no instruction-likeness. The anytime-valid framework is
attack-agnostic; the particular signal is not, and provenance- or
transformation-aware signals are the foremost future work. Adaptive adversaries
minimising instruction-likeness are a special case of the same weakness.
\emph{(iii)~Single attack \textbf{corpus}}: both attacks come from the
AgentDojo/InjecAgent ecosystem; DoS and tool-knowledge templates remain
untested. Three limitations of earlier versions are now \emph{resolved}: a
saturated PAC threshold and single-digit samples (fixed by the \(121\)-trajectory
collection, so the PAC bound~\eqref{eq:pac} is certified), and the ``single
agent'' concern---SENTRY transfers cleanly to a second, unrelated agent
(GPT-4o-mini; AUROC \(0.82\)).
...
By contrast, LLM-judge detectors add a full model call per step and
re-execution defenses~\cite{zhu2025melon} re-run the entire agent, both
requiring a GPU or API that SENTRY does not.
...
\paragraph{Responsible use.} A monitor is a mitigation, not a guarantee: an
adaptive adversary could phrase an injection to minimise instruction-likeness,
and defenses must be evaluated against such adaptive
attacks~\cite{nasr2025attackersecond}. SENTRY is best deployed in
depth---alongside provenance-based prevention~\cite{debenedetti2025camel} and
output sanitisation~\cite{das2025commandsans}---as a statistically controlled,
model-agnostic tripwire rather than the sole line of defense.
...
All hyperparameters (target FAR $\alpha=0.2$, PAC parameter $\delta=0.2$, novelty and instruction weights $=4$, instruction-likeness weights $(3,2,2,1)$, $K=6$ tilt components, $10$ seeds, two-action length filter), the extended signal definitions, and a comparison of the Shiryaev--Roberts and CUSUM recursions are given in the supplementary material. Every number and figure is regenerated by deterministic scripts (seed-stable CRC feature hashing) released with the project repository; randomness enters only through the ten train/calibration/threshold/test splits.

\section*{Acknowledgments}
The author thanks the maintainers of AgentDojo, $\tau$-bench and the
e-detector and e-value software ecosystems. All code, the collected
trajectory corpus, and the scripts that reproduce every reported number and
figure are released with the project repository. No external funding was
received and the author declares no competing interests.

\bibliographystyle{IEEEtran}
\bibliography{references_ieee}

% Author biography is added at camera-ready (kept out of the 12-page initial
% submission per the standard TDSC page limit):
% \begin{IEEEbiographynophoto}{Quang-Vinh Dang}
% is with British University Vietnam, Hung Yen, Vietnam. His research interests
% include machine learning, anytime-valid and sequential statistical inference,
% and the safety and security of large-language-model-based agents.
