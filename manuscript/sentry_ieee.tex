...
they did not separate on our data---and are explicit about the ceiling imposed
by trace observability and by evaluation on a single agent and attack family.
...
agent chooses what to surface) could close it. \emph{(ii)~One attack family}:
we evaluate AgentDojo's \texttt{important\_instructions} attack across four
suites; generality across attack templates and against \emph{adaptive}
adversaries that phrase injections to minimise instruction-likeness is untested
and is the most important robustness question. \emph{(iii)~Single agent}: all
trajectories come from DeepSeek-V4-Flash; whether the nominal reference
transfers across agent models is open. Note that the two limitations that
dominated an earlier small-sample version of this work---a saturated PAC
threshold and single-digit sample sizes---are \emph{resolved} here: with \(121\)
nominal trajectories the Binomial-tail order statistic is non-degenerate and
the PAC bound~\eqref{eq:pac} is a genuine certified guarantee.
