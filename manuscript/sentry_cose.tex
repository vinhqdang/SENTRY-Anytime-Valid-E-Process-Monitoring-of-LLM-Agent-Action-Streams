...
earlier, unpublished draft of this work, where it cost $0.25$ AUROC and produced an
explanation we had written up and believed, which was wrong. Second, we show that action-side
...
monitoring---the fraction $\nu$ of compromises leaving no trace evidence bounds
every such monitor---and then show that on AgentDojo it does not bind: $\nu$ lies
between $0$ and $0.044$ depending on how payload visibility is defined, placing
the bound at or above $0.958$, because the benchmark delivers payloads through tool
outputs a successful injection must have read. Our residual error is therefore
scoring error, not missing information; and since $\nu$ depends on a measurement
convention as well as on the corpus, cross-corpus rates are incomparable unless
both are reported. Fourth, we document a
...
trace evidence. Measuring that fraction on AgentDojo puts the bound at or above
$0.958$ under every criterion we tried, so it does not bind: injections arrive
through tool outputs a successful attack must have read. We also show the estimate
is sensitive to how ``visible'' is defined, which is why we withdraw an earlier
claim that a published figure exceeded this corpus's information limit.
...
them---which an earlier, unpublished draft of this work included in the
...
split. An earlier, unpublished draft of this work folded S1 and S2 into a single score as
...
We must be precise about where this has and has not been undone.
...
and~\ref{tab:longhorizon}) are computed with the project's original scalar
pipeline, which sums the per-action signals with fixed coefficients
$(1, 4, 4, 4)$. We have not re-derived those tables, because in each of them the
weighting is held constant while one factor varies---the normalisation in
Table~\ref{tab:normablation}, the agent or attack family in
...
an earlier, unpublished draft used, is to subtract the largest value observed in
...
(ii) $\psi$ is invariant to any strictly increasing reparameterisation of the
signal; and (iii) $\psi$ takes at most $n+1$ distinct values, all in
...
transform yields $0.925$ at $n=5$, $0.979$ at $n=23$, $0.986$ at $n=35$ and
$0.992$ at $n=63$ (400 replicates each; the calibration sizes used in this paper
are $23$ to $63$). The loss is $O(1/n)$, which is why we prefer the largest
...
\begin{algorithmic}[1]
\Require disjoint benign splits $\Dcal_{\mathrm{fit}}, \Dcal_{\mathrm{cal}}, \Dcal_{\mathrm{thr}}$; target false-alarm rate $\alpha$; signals $1,\dots,K$
...
\EndFor
\State $\tau \gets$ threshold realising a false-alarm rate $\le \alpha$ on $\{S(\Tcal) : \Tcal \in \Dcal_{\mathrm{thr}}\}$ (Section~\ref{sec:metrics})
...
kept separate, S1 and S2 collapse to $0.48$--$0.56$
...
would then have been measured at over $5\%$. An earlier, unpublished draft of this work
...
Table~\ref{tab:polarity} settles the direction that matters: every one of the $31$
...
on GPT-4o-mini and $0.780 \pm 0.021$ on DeepSeek (Table~\ref{tab:fused}). We had
...
attainable at these sample sizes (Section~\ref{sec:metrics}). The rows therefore
sit at \emph{different} operating points---from $0.0\%$ to $4.4\%$---so the TPR
column is not a like-for-like comparison across rows, whereas AUROC, being
threshold-free, is. SENTRY-Fuse in bold.}
\label{tab:fused}
\footnotesize
...
$0.862$ and $0.836$ AUROC, and their recall at their own much stricter realised
operating points ($0.0\%$ and $0.8\%$) is near zero. They contribute cases the
observation signal misses rather than duplicating it, which is why we rest the
claim on AUROC---it does not depend on where each row's threshold happens to
fall. This
...
these two signals do not belong in the monitor. An earlier, unpublished draft of this work
...
on this benchmark the limit turns out not to bind.
...
\paragraph{The measurement, and why it is criterion-dependent}
Measuring $\nu$ requires deciding when the payload counts as \emph{visible}, and
this turns out to matter more than we expected. AgentDojo's
\texttt{important\_instructions} payloads share a fixed wrapper: across all $110$
payloads here the first $32$ words and the last $21$ are identical, so a test that
looks for the payload's opening text is largely testing for scaffolding, while one
that demands the attacker's specific directive verbatim is partly testing how the
logging renderer reflowed it. We therefore report two criteria and the sensitivity
in between (Table~\ref{tab:ceiling}).

Under the \emph{permissive} criterion---any contiguous eight-word span of the
payload appears in some tool output---every compromised trajectory is visible and
$\nu = 0$. Under the \emph{strict} criterion---the payload's opening eight words
appear verbatim---$0.867$ are visible and $\nu = 0.044$ pooled. The attacker's
distinctive directive, with the wrapper stripped, survives verbatim for at least
five contiguous words in \emph{all} $90$ cases, for at least ten in $52\%$, and in
full in only $14\%$.

\begin{corollary}\label{cor:ceiling}
Pooled, $\nu \in [0, 0.044]$ across these criteria, so at $\alpha = 0.05$
Theorem~\ref{thm:ceiling} gives $\mathrm{TPR} \le 1$ under the permissive reading
and $\mathrm{TPR} \le 0.958$ under the strict one. Our pooled $0.919$ lies below
the bound on either reading, so no conclusion in this paper depends on which is
adopted.
\end{corollary}

\begin{table}[t]
\centering
\caption{Where evidence of compromise lives. The observation channel is measured
two ways---permissively (any eight-word payload span visible) and strictly (the
payload's opening eight words visible)---because the payloads share a fixed
wrapper and the two readings disagree. $\nu$ is the fraction with neither channel;
the lower block gives the fraction whose \emph{distinctive} directive, with the
shared wrapper removed, survives verbatim for at least $k$ contiguous words.}
\label{tab:ceiling}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
 & GPT-4o-mini & DeepSeek & Pooled \\
 & ($n=72$) & ($n=18$) & ($n=90$) \\
\midrule
...
\midrule
$\nu$, permissive & $0.000$ & $0.000$ & $0.000$ \\
$\nu$, strict     & $0.028$ & $0.111$ & $0.044$ \\
\midrule
\multicolumn{4}{@{}l}{\emph{distinctive directive visible for $\ge k$ words}} \\
...
\caption{Evidence-channel coverage of the compromised class, under the permissive
visibility criterion of Table~\ref{tab:ceiling}. The observation channel is
saturated on both corpora, so the bound of Theorem~\ref{thm:ceiling} does not bind;
under the strict criterion it falls to $0.889$ and $0.778$ and the bound is still
$0.958$ or above. The action channel is not saturated, which is why sink
provenance contributes recall rather than replacing the observation signal.}
\label{fig:evidence}
\end{figure}

Three consequences follow, and they are not the ones we anticipated.

\paragraph{The ceiling is at most weakly binding, and it is a property of the
benchmark} On either criterion the bound sits at or above $0.958$, well above what
any detector here achieves, so it does not explain our residual error. The reason
is structural: AgentDojo delivers its payload \emph{through a tool output}, and an
injection cannot succeed unless the agent read that output. Any benchmark built
this way will have $\nu$ near zero by construction. A trace-only monitor evaluated
on such a corpus is limited by its \emph{scoring}, not by the information
available---so the $8\%$ we miss is signal we failed to extract, not signal that
was absent. That is a more demanding conclusion than a binding ceiling would have
licensed.

\paragraph{This is a caution about benchmark design}
The corpora the field uses cannot exhibit the invisible-compromise regime, so a
defence validated only on them has not been tested against it. Real deployments
admit compromises that a benchmark of this shape excludes: an agent persuaded
through a channel the monitor never sees, or one whose harmful action is
indistinguishable from a legitimate one. We think building a corpus in which some
successful injections genuinely leave no trace evidence is the more useful
follow-up to this section than any improvement to our detector.

\paragraph{A headline $\nu$ should not be quoted without its criterion}
Our own experience is the argument. The same $90$ trajectories yield $\nu = 0$ or
$\nu = 0.044$ depending on a decision about text matching that looks like an
implementation detail, and an earlier, unpublished draft of this work reported $\nu = 0.078$
from a third variant and drew a substantive conclusion from it---that a published
figure exceeded the corpus's information limit. That conclusion was an artefact of
the criterion, and we withdraw it. What survives is the weaker and more useful
recommendation: because $\nu$ is a property of a corpus \emph{and} of a
measurement convention, a true-positive rate on one corpus is not comparable with
a rate on another unless both report $\nu$ and how they computed it.
...
part of the method in an earlier, unpublished draft of this work. Under the three-way split
...
task''. Its semantic replacement, S4, works ($0.900$ on GPT-4o-mini, $0.780$ on
...
identical signal. A hand-picked threshold dominates the e-detector on both axes at
every level the e-detector can certify.}
...
Two findings stand out. A fixed threshold on the same score dominates the
e-detector at every level the e-detector can actually certify
($\alpha \le 0.05$)---fewer false alarms and higher detection simultaneously. Only
at $\alpha = 0.2$, where the guarantee is $\mathrm{ARL} \ge 5$ and therefore
worthless, does the e-detector achieve a higher detection rate ($0.624$ against
$0.588$), and it does so at nearly three times the false-alarm cost. And the e-detector's results are \emph{identical} for
...
otherwise: on AgentDojo nearly every compromise leaves observation-side evidence,
the bound sits at or above $0.958$, and the $8\%$ we miss is scoring error rather
than missing information. The
...
invisible to all three signals. Because $\nu$ is near zero on this corpus, almost
no such case appears in our positives---which is a limitation of the corpus rather
...
measure $1.000 \to 0.986$ on separable synthetic data, and $1.000 \to 0.925$ at
$n=5$. It can also, in adverse configurations, take an above-chance signal below
...
corpus it does not bind in any case.
...
$\nu$ of compromises leaving no trace evidence bounds every trace-only
monitor---but the measurement refutes the expectation. On AgentDojo $\nu$ lies
between $0$ and $0.044$ depending on how payload visibility is defined, putting the
bound at or above $0.958$, because the benchmark delivers its payloads through tool
outputs a successful injection must have read. The ceiling is therefore an artefact
of benchmark construction rather than a limit of the problem, and the $8\%$ we miss
is scoring error rather than absent evidence. We also found $\nu$ to be
uncomfortably sensitive to a text-matching convention---the same $90$ trajectories
give $0$, $0.044$ or, under a variant used in an earlier draft, $0.078$---so we
withdraw the claim that a published figure exceeded this corpus's information
limit, and recommend instead that papers report $\nu$ together with the criterion
used, and that someone build a corpus in which compromises really can hide.
